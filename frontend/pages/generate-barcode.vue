      ...
      <v-row>
        ...
        <v-col cols="12" md="6">
          ...
          <v-card v-else min-height="300" class="d-flex align-center justify-center">
            <span class="text-grey">バーコードを生成するには左側の設定を行い、「バーコードを生成」ボタンをクリックしてください</span>
          </v-card>
        </v-col>
      </v-row>
    </v-card>
...
const generateBarcode = async () => {
  try {
    ...
    const response = await axios.post('/api/barcode/generate', params);
...
const printBarcode = () => {
  ...
  printWindow.document.write('<div style="margin-bottom: 20px;"><img src="' + barcodeData.value.image_data + '" alt="バーコード"></div>');
